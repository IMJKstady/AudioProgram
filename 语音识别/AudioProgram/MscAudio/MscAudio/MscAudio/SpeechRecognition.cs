...
namespace MscAudio {
    ...
    public class SpeechRecognition {
        ...
        // 识别句柄
        private IntPtr m_Ptr = default;

        private string @params = "sub = iat, domain = iat, language = zh_cn, accent = mandarin, sample_rate = 16000, result_type = plain, result_encoding = UNICODE";
        ...
        public SpeechRecognition(string path) {
            ...
            if (m_Datas == null) Console.WriteLine("没有读取到数据");
        }
        ...
        private void ThreadStart() {
            ...
            if (errorCode != ErrorCode.MSP_SUCCESS) {
                Console.WriteLine("开始识别出错：（错误码）" + (int) errorCode);
                Account.Logout();
                return;
            }
            ...
            for (int i = 0; i < tims; i++) {
                ...
                if (errorCode1 != ErrorCode.MSP_SUCCESS) {
                    Console.WriteLine("音频数据写入失败:（错误码）" + (int) errorCode1);
                    Account.Logout();
                    return;
            ...
            while (status != RsltStatus.MSP_REC_STATUS_COMPLETE) {
                ...
                if (errorCode2 != ErrorCode.MSP_SUCCESS) {
                    Console.WriteLine("获取失败：（错误码）" + (int) errorCode2);
                    Account.Logout();
                    return;
                }
            ...
            if (errorCode3 != ErrorCode.MSP_SUCCESS) {
                Console.WriteLine("结束失败：（错误码）" + (int) errorCode3);
            }
